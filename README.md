...
src/
  ...
  styles/        tokens.css (primitives -> semantic -> component), components.css,
                 map.css, fonts.css
scripts/         e2e.mjs (the Playwright pass), shot.mjs (screenshots),
                 print.mjs (the 11x17 PDF/PNG), extract-blobs.py, build-booths.py
print/           generated by print.mjs, not committed
...
The 11x17 handout is the same data on paper, not a separate drawing.
`/?print=1` renders `src/print/PrintSheet.jsx`: the whole festival at booth
numbers, a legend, the booth-range key, the food vendor list, and an
alphabetical artist index. `npm run print` builds and writes
`print/fall-fest-map-2026.pdf` (vector, 17x11in) and `.png` (300dpi) with the
bundled Brice and Manrope embedded; open `/?print=1` in a browser to proof it.

Files go to the printer Tuesday 9/22. Before that run, re-read Courtney's
sheet and re-check `vendors.json` for null locations (the sheet lists vendors
by name only, so a null there costs nothing, but the note in that file stands).
